...
class LoginViewController: BaseViewController {
    ...
    func createUser() {
        let user1 = User(name: "hungnguyen", password: "123")
        let user2 = User(name: "stevjob", password: "234")
        let user3 = User(name: "billgate", password: "345")
    ...
    func checkUserAuth(){
        ...
        if isAuthenticated == true{
            // khoi tao 1 object cua class BaseTabbarController
            let tabbar = BaseTabbarController()
            
            // hien thi 1 tabbar khi isAuthenticated = true
